...
contract Voluntary_project {
   ...
   function insert_degree(bytes calldata degree_, bytes calldata description_, address[] calldata verifiers) external only_valid_client{
    ...
    function determineAmount_toPay_to_ageValidator (address client, uint indx) external view returns (uint to_pay) {
        
        uint k = valid_clients[client].age[client].verification_res[indx].counter;
        to_pay = (determin_total_share (k));
    }
    
    function pay_validator_of_age(address client, uint indx) payable external{
        ...
        uint k = valid_clients[client].age[client].verification_res[indx].counter++;
        //uint total_numOf_parties = k; //total number of parties that get paid.//recently added
        uint total_amount_to_pay = (determin_total_share (k));
        //temp = total_amount_to_pay;
        require(msg.value >= k); // ensure the user pays enough coin
