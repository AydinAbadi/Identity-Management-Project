...
contract Voluntary_project {
    ...
    function find_age(address client, bytes calldata age_)external view returns (bool found){

        if(keccak256(valid_clients[client].age[client].val) == keccak256(age_))
            found = true;
    }

    function find_degree_index(address client, bytes calldata degree_type,  bytes calldata desc) external view returns (bool found, uint index){
    ...
    function find_license_index(address client, bytes calldata license_type,  bytes calldata desc, bytes calldata exp_date) external view returns (bool found, uint index){
