...
contract Voluntary_project {
    ...
    function get_client_age_verification_res(address client, uint indx) external view returns (bytes32){
     ...
     function get_client_age_howmany_getsPaid(address client, uint indx) external view returns (uint){
        
        return valid_clients[client].age[client].verification_res[indx].counter;
    }
    
    function get_client_age_whoElse_getsPaid(address client, uint indx,uint counter) external view returns (address){
        
        return valid_clients[client].age[client].verification_res[indx].who_gets_paid[counter];
    }
   ...
   function insert_degree(bytes calldata degree_, bytes calldata description_, address[] calldata verifiers) external only_valid_client{
    ...
    //in future we need to add a feature to this function, so the client can provide a signature on the opening of res to this function
    // so the function first checks it's the siganture of the validator, and then verifies the commitment.
    
    // this function allows one to report an inconsistent result provided by a validator. 
    //(r=random value,m=message commited/age):opening of the commitment
    ...
    function pay_age_validator(address client, uint indx) payable external{
        
        //pay the validator
        uint counter = valid_clients[client].age[client].verification_res[indx].counter;
        uint total_numOf_parties = counter + 1; //total number of parties that get paid.
        uint total_amount_to_pay = determin_total_share (total_numOf_parties);
        require(msg.value >= total_amount_to_pay); // ensure the user pays enough coin
        uint each_share = (total_amount_to_pay) / total_numOf_parties;
        valid_clients[client].age[client].verification_res[indx].verifier_addr.transfer(each_share);
        // it pays those who already paid
        for(uint i = 0; i < counter; i++){
            valid_clients[client].age[client].verification_res[indx].who_gets_paid[i].transfer(each_share);
        }
        uint k = valid_clients[client].age[client].verification_res[indx].counter++;
        valid_clients[client].age[client].verification_res[indx].who_gets_paid[k] = msg.sender;
    }
    ...
    function validate_degree(address client, bytes32 res, uint index) external{
       ...
       require(valid_clients[client].valid == true);
       // check if the verifier is valid.
       require(valid_clients[client].degree[index].verifier_exists[msg.sender] == true);
       require(valid_clients[client].degree[index].verifiers_verified[msg.sender] == false);
