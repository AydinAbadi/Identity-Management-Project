...
contract Voluntary_project {
    ...
    // for the current version, indx is 0, as there's only one verifier allowed to provide its input.
    ...
    function pay_validator_of_degree(address client, uint indx) payable external{
        ...
        k = k-1;
        uint t = valid_clients[client].degree[indx].verification_res[k].counter++;
        uint total_amount_to_pay = (determin_total_share (t));
        require(msg.value >= total_amount_to_pay); // ensure the user pays enough coin
        uint each_share = (total_amount_to_pay) / t;
        for(uint i = 0; i < t; i++){
            valid_clients[client].degree[indx].verification_res[k].who_gets_paid[i].transfer(each_share) ;
        }
        valid_clients[client].degree[indx].verification_res[k].who_gets_paid[t] = msg.sender;
    }
    ...
    function pay_validator_of_license(address client, uint indx) payable external{
        ...
        require(k > 0);// ensures that at least one validator has provided its input.
        k = k-1;
        uint t = valid_clients[client].license[indx].verification_res[k].counter++;
        uint total_amount_to_pay = (determin_total_share (t));
        require(msg.value >= total_amount_to_pay); // ensure the user pays enough coin
        uint each_share = (total_amount_to_pay) / t;
        for(uint i = 0; i < t; i++){
            valid_clients[client].license[indx].verification_res[k].who_gets_paid[i].transfer(each_share) ;
        }
        valid_clients[client].license[indx].verification_res[k].who_gets_paid[t] = msg.sender;
    }
