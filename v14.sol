...
contract Voluntary_project {
    ...
    uint public total_valid_clients; // total number of client registered by the owner/admin
    mapping (uint => address) public valid_client_list;// list of valid clients-- it retruns the clients' address

    uint public total_valid_validators;// total number of validator registered by the owner/admin
    mapping (uint => address) public valid_validators_list;// list of valid validators-- it retruns the validators' address

    uint public total_valid_admins; // total number of admin registered by the owner
    mapping (uint => address) public valid_admins_list; // list of valid admin-- it retruns the admins' address
    ...
    struct Age{
        ...
        bytes val;
        bytes file_hash;// a commitment to the hash of a file is stored here.
    ...
    function get_client_age_val(address client) external view returns (bytes memory ){
     ...
     function get_client_age_committed_fileHash(address client) external view returns (bytes memory ){

        return  valid_clients[client].age[client].file_hash;
    }
    ...
    function get_client_age_verification_res(address client, uint indx) external view returns (bytes32){
      ...
      function get_client_age_verification_verifier(address client, uint indx) external view returns (address){

        return valid_clients[client].age[client].verification_res[indx].verifier_addr;
    }
    ...
    //given an encrypted approval:encrypted_approval (computed locally), it checks if it matches the one stored in the contract for a certain client and verifier
    function compare_client_age_verification_res(address client, address verifier, bytes32 encrypted_approval)external view returns (bool res_) {
        ...
        for(uint i  = 0;i < k; i++){
            ...
            && keccak256(abi.encodePacked(valid_clients[client].age[client].verification_res[i].res)) == keccak256(abi.encodePacked(encrypted_approval))){
             res_ = true;
             return res_;
            }
        }
    ...
    function get_client_degree_committed_fileHash(address client, uint indx) external view returns (bytes memory){

        return valid_clients[client].degree[indx].file_hash;
    }
    ...
    //given an encrypted approval:encrypted_approval (computed locally), it checks if it matches the one stored in the contract for a certain client and verifier
    function compare_client_degree_verification_res(address client, address verifier, uint indx, bytes32 encrypted_approval)external view returns (bool res_) {
        
        uint k = valid_clients[client].degree[indx].total_number_of_verifications;
        for(uint i  = 0;i < k; i++){
            
            if (valid_clients[client].degree[indx].verification_res[i].verifier_addr == verifier
            && keccak256(abi.encodePacked(valid_clients[client].degree[indx].verification_res[i].res)) == keccak256(abi.encodePacked(encrypted_approval))){
             res_ = true;
             return res_;
            }
        }
    ...
    function get_client_license_committed_fileHash(address client, uint indx) external view returns (bytes memory){

        return valid_clients[client].license[indx].file_hash;
    }
    ...
    function compare_client_license_verification_res(address client, address verifier, uint indx, bytes32 encrypted_approval)external view returns (bool res_) {
        
        uint k = valid_clients[client].license[indx].total_number_of_verifications;
        for(uint i  = 0;i < k; i++){
            
            if (valid_clients[client].license[indx].verification_res[i].verifier_addr == verifier
            && keccak256(abi.encodePacked(valid_clients[client].license[indx].verification_res[i].res)) == keccak256(abi.encodePacked(encrypted_approval))){
             res_ = true;
             return res_;
            }
        }
    ...
    function find_age(address client, bytes calldata age_, bytes calldata file_hash_)external view returns (bool found){

        if(keccak256(valid_clients[client].age[client].val) == keccak256(age_)
        && keccak256(valid_clients[client].age[client].file_hash) == keccak256(file_hash_))
            found = true;
    }

    function find_degree_index(address client, bytes calldata degree_type,  bytes calldata desc, bytes calldata file_hash_) external view returns (bool found, uint index){

        uint j = valid_clients[client].total_number_of_degrees;
        for(uint i = 0;i < j; i++){
            if(keccak256(valid_clients[client].degree[i].type_) == keccak256(degree_type) 
            && keccak256(valid_clients[client].degree[i].description) == keccak256(desc)
            && keccak256(valid_clients[client].degree[i].file_hash) == keccak256(file_hash_))
            {
                found = true;
                index = i;
                break;
    ...
    function find_license_index(address client, bytes calldata license_type,  bytes calldata desc, bytes calldata exp_date, bytes calldata file_hash_) external view returns (bool found, uint index){

        uint j = valid_clients[client].total_number_of_licenses;
        for(uint i = 0;i < j; i++){
            if(keccak256(valid_clients[client].license[i].type_) == keccak256(license_type)
            && keccak256(valid_clients[client].license[i].description) == keccak256(desc)
            && keccak256(valid_clients[client].license[i].Expiry_date) == keccak256(exp_date)
            && keccak256(valid_clients[client].license[i].file_hash) == keccak256(file_hash_))
            {
                found = true; 
                index = i;
                break;
    ...
    function register_validator(address validator) external only_valid_admin{ // registers valid validators in the smart contract.
        ...
        valid_validators_list[total_valid_validators] = validator;
        total_valid_validators++;
    }
    ...
    function register_client(address client) external only_valid_admin{ // registers valid volunteers in the smart contract.
        ...
        valid_client_list[total_valid_clients] = client;
        total_valid_clients++;
    }
    ...
    function add_admin(address new_admin) external only_owner{
        ...
        valid_admins_list[total_valid_admins] = new_admin;
        total_valid_admins++;
    }
    ...
    function insert_age(bytes calldata com_val_, bytes calldata com_fileHash_ , address verifier_1, address verifier_2, address verifier_3, address verifier_4) external only_valid_client{

        address[4] memory verifiers= [verifier_1, verifier_2, verifier_3, verifier_4];
        ...
        if(valid_clients[msg.sender].age[msg.sender].inserted == false){
            ...
            valid_clients[msg.sender].age[msg.sender].file_hash = com_fileHash_;
        }
    }

    function insert_degree(bytes calldata degree_, bytes calldata description_, bytes calldata com_fileHash_ , address verifier_1, address verifier_2, address verifier_3, address verifier_4) external only_valid_client{
        ...
        valid_clients[msg.sender].degree[j].file_hash = com_fileHash_;
    }

    function insert_license(bytes calldata license_, bytes calldata description, bytes calldata expiry_date, bytes calldata com_fileHash_, address verifier_1, address verifier_2, address verifier_3, address verifier_4) external only_valid_client{
        ...
        valid_clients[msg.sender].license[j].file_hash = com_fileHash_;// it stores a committment to the hash of the file.
    }
